...
module overmind::over_network {
    ...
    */
    fun init_module(admin: &signer) {
        ...
        let nft_collection_ref = collection::create_unlimited_collection(
            &resource_account,
        ...
        let collection_address = object::address_from_constructor_ref(&nft_collection_ref);
        ...
        move_to(&resource_account, ModuleEventStore {
            account_created_events: account::new_event_handle<AccountCreatedEvent>(&resource_account),
            account_follow_events: account::new_event_handle<AccountFollowEvent>(&resource_account),
            account_unfollow_events: account::new_event_handle<AccountUnfollowEvent>(&resource_account),
            account_post_events: account::new_event_handle<AccountPostEvent>(&resource_account),
            account_comment_events: account::new_event_handle<AccountCommentEvent>(&resource_account),
            account_like_events: account::new_event_handle<AccountLikeEvent>(&resource_account),
            account_unlike_events: account::new_event_handle<AccountUnlikeEvent>(&resource_account)
        });

        move_to(&resource_account, GlobalTimeline {
    ...
    */
    entry fun update_bio(
        ...
        // check if the bio is not too long or not
        check_the_bio_is_not_too_long(bio);
    ...
    */
    inline fun get_resource_account_address(): address {
        // Create the module's resource account address and return it
    ...
    // check if the bio is not too long or not.
    inline fun check_the_bio_is_not_too_long (bio: String) {
        assert!(string::length(&bio) >= 0 && string::length(&bio) < 100, EBioInvalidLength);
    }
